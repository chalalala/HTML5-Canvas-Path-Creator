class Program {
	...
	public initialize() {
		...
		bindCheckbox(
			"#showConstructionDotsOnLastClickedPointOnly",
			"showConstructionDotsOnLastClickedPointOnly"
		);
		...
		$("#insert-point").on("click", () => {
			const positionX = $("#insert-point-x").val();
			const positionY = $("#insert-point-y").val();

			const point: Point = {
				x: typeof positionX === "string" ? parseInt(positionX) : 0,
				y: typeof positionY === "string" ? parseInt(positionY) : 0,
		...
		const moveMovingPt = (point: Point, isShiftPressed: boolean) => {
			if (this.movingPt.relativeTo) {
				...
				if (isShiftPressed) {
					...
					const angle =
						Math.atan2(this.movingPt.y, -this.movingPt.x) + Math.PI;
					...
					let otherDistance = Math.sqrt(
						newPt.x * newPt.x + newPt.y * newPt.y
					);
					/*if(oppositeMouse) {
                        otherDistance *= -1;
                    }*/
		...
		for (const point of this.points) {
			switch (point.type) {
				...
				case PointType.BEZIER_CURVE:
					if (lastPoint) {
						const absoluteC2 = this.addPointsTogether(
							lastPoint,
							lastPoint.c2
						);
						...
						addLine(
							`ctx.bezierCurveTo(${xCoord(absoluteC2.x)}, ${yCoord(
								absoluteC2.y
							)}, ${xCoord(absoluteC1.x)}, ${yCoord(
								absoluteC1.y
							)}, ${xCoord(point.x)}, ${yCoord(point.y)})`
						);
					} else {
						addLine(
							`ctx.moveTo(${xCoord(point.x)}, ${yCoord(point.y)});`
						);
					}
					break;
				...
				case PointType.QUADRATIC_CURVE:
					...
					addLine(
						`ctx.quadraticCurveTo(${xCoord(absoluteC3.x)}, ${yCoord(
							absoluteC3.y
						)}, ${xCoord(point.x)}, ${yCoord(point.y)});`
					);
					break;
	...
	protected redraw() {
		...
		if (this.options.showDots) {
			...
			for (const point of this.points) {
				...
				if (
					!this.options.showConstructionDotsOnLastClickedPointOnly ||
					this.lastClickedPoint === point
				) {
	...
	protected selectedPoints(location: Point, precision?: number) {
		...
		const addIfValid = (point) => {
			if (
				...
				(this.options.showConstructionDotsOnLastClickedPointOnly &&
					point.relativeTo &&
					point.relativeTo !== this.lastClickedPoint)
			) {
				return;
			}
	...
	protected removePoint(point: Point, useInstance?: boolean) {
		for (let i = 0; i < this.points.length; ++i) {
			...
			if (
				(!useInstance && pt.x === point.x && pt.y === point.y) ||
				(useInstance && pt === point)
			) {
		...
		if (cursorPoint && cursorPoint.relativeTo) {
			...
			if (adjustPoint) {
				...
				const otherDistance = Math.sqrt(
					adjustPoint.x * adjustPoint.x + adjustPoint.y * adjustPoint.y
				);
